...
cgssStreamSeek(CGSS_HANDLE handle, int64_t offset, CGSS_STREAM_SEEK_ORIGIN origin) {
    CHECK_HANDLE(handle);
    try {
...
cgssStreamCopyTo2(CGSS_HANDLE source, CGSS_HANDLE destination, uint32_t bufferSize) {
    CHECK_HANDLE(source);
    CHECK_HANDLE(destination);
    try {
...
cgssCreateFileStream(const char *fileName, _OUT_ CGSS_HANDLE *stream) {
    if (!stream) {
...
cgssCreateFileStream2(const char *fileName, CGSS_FILE_MODE fileMode, _OUT_ CGSS_HANDLE *stream) {
    if (!stream) {
        return CGSS_OP_INVALID_ARGUMENT;
    }
    alloc_stream(
        stream, new CFileStream(fileName, static_cast<FileMode>(fileMode)), HandleType::CStream
    );
    return CGSS_OP_OK;
}
...
cgssCreateFileStream3(
    const char *fileName,
    CGSS_FILE_MODE fileMode,
    CGSS_FILE_ACCESS fileAccess,
    ...
    alloc_stream(
        stream,
        new CFileStream(
            fileName, static_cast<FileMode>(fileMode), static_cast<FileAccess>(fileAccess)
        ),
        HandleType::CStream
    );
    return CGSS_OP_OK;
...
cgssCreateHcaDecoder(CGSS_HANDLE baseStream, _OUT_ CGSS_HANDLE *decoder) {
    ...
    alloc_stream(
        decoder,
        new CHcaDecoder(to_stream(baseStream)),
...
cgssCreateHcaDecoder2(
    CGSS_HANDLE baseStream, const HCA_DECODER_CONFIG *decoderConfig, _OUT_ CGSS_HANDLE *decoder
) {
    CHECK_HANDLE(baseStream);
    ...
    alloc_stream(
        decoder,
        new CHcaDecoder(to_stream(baseStream), *decoderConfig),
        HandleType::CStream | HandleType::CHcaReaderBase
    );
    return CGSS_OP_OK;
}
...
cgssCreateCipherConverter(
    CGSS_HANDLE baseStream,
    ...
    alloc_stream(
        converter,
        new CHcaCipherConverter(to_stream(baseStream), *cryptFrom, *cryptTo),
        HandleType::CStream | HandleType::CHcaReaderBase
    );
    return CGSS_OP_OK;
}
...
CGSS_API_IMPL(CGSS_OP_RESULT) cgssGetHcaInfo(CGSS_HANDLE handle, HCA_INFO *info) {
    ...
    auto *reader =
        dynamic_cast<CHcaFormatReader *>(CHandleManager::getInstance()->getHandlePtr(handle));
    HCA_INFO &i = *info;
    reader->GetHcaInfo(i);
    return CGSS_OP_OK;
}
...
cgssUtfReadTable(CGSS_HANDLE stream, uint64_t offset, UTF_TABLE **table) {
    CHECK_HANDLE(stream);
    ...
    *table = nullptr;
    ...
    for (auto &row : rows) {
        ...
        std::sort(fieldList.begin(), fieldList.end(), [](const UTF_FIELD &left, const UTF_FIELD
        &right) { if (left.offsetInRow < right.offsetInRow) { return true; } else if
        (left.offsetInRow > right.offsetInRow) { return false; } else { return left.offset <
        right.offset;
            }
        });
